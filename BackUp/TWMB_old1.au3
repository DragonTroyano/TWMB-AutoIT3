...
;Globalne zmienne
Global $WorldsN = 91 ;IloúÊ úwiatÛw
...
;Grupy
$gMain = GUICtrlCreateGroup("Panel GlÛwny", 10, 10, 700, 130)
...
;		MsgBox(0, "", $Date)
   ...
   If $Work = 1 And $Launch > 0 Then
	If _DateDiff("s", INIRead($CDataINI, $IDattack, "Launch", "0"), $Date&" "&$Time) >= 0 Then
			$oAtt = _IEGetObjById($oIE, "troop_confirm_go")
			_IEAction($oAtt, "Click")
			If $Attacks >= 2 Then
			$oAtt = _IEGetObjById($oIE2, "troop_confirm_go")
			_IEAction($oAtt, "Click")
			EndIf
			If $Attacks >= 3 Then
			$oAtt = _IEGetObjById($oIE3, "troop_confirm_go")
			_IEAction($oAtt, "Click")
			EndIf
			If $Attacks >= 4 Then
			$oAtt = _IEGetObjById($oIE4, "troop_confirm_go")
			_IEAction($oAtt, "Click")
			EndIf
			If $Attacks >= 5 Then
			$oAtt = _IEGetObjById($oIE5, "troop_confirm_go")
			_IEAction($oAtt, "Click")
			EndIf
...
;		MsgBox(0, Null, "Index: "&$Index&@CRLF&"Ataki: "&$Attacks&@CRLF&"IDAttack2 = "&$AttackID2&@CRLF&"IDAttack3 = "&$AttackID3&@CRLF&"IDAttack4 = "&$AttackID4&@CRLF&"IDAttack5 = "&$AttackID5)
   ...
   If $URL <> $URL1 And $URL <> "http://www."&GUICtrlRead($Country)&"/sid_wrong.php" and $URL <> "http://www."&GUICtrlRead($country) And $Work = 1 Then
	   $URL1 = $URL
   If StringRight(_IEPropertyGet($oIE0, "locationurl"), StringLen("&screen=info_village")) = "&screen=info_village" Then
	   GUICtrlSetStyle($NAVIGATE_Button_Copy, "")
	   GUICtrlSetStyle($NAVIGATE_Button_Add, "")
   ElseIF StringRight(_IEPropertyGet($oIE0, "locationurl"), StringLen("&screen=info_village")) <> "&screen=info_village" Then
...
Local $sSearch = "screen=info_village"
Local $sTxt = $iNumLinks & " links found" & @CRLF & @CRLF
$LP = 0
For $oLink In $oLinks
   ;$sTxt &= $oLink.href & @CRLF
   If StringRight($oLink.href, StringLen($sSearch)) = $sSearch Then
